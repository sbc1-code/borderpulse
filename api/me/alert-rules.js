...
import { validateAlertRuleInput, validateAlertRulePatchInput } from '../_lib/validation.js';
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'PATCH') {
      if (!id) throw new HttpError(400, 'id is required');
      const input = validateAlertRulePatchInput(requestBody(req));
      if (input.saved_crossing_id) {
      ...
      if (input.enabled === true) {
        ...
        if (!profile?.email_opt_in) throw new HttpError(409, 'Email opt-in is required before enabling an alert');
      }
      const { data, error } = await client
        .from('alert_rules')
        .update(input)
      ...
      if (error) throw new Error(error.message);
      if (!data) throw new HttpError(404, 'Alert rule not found');
      json(res, 200, { alert_rule: data });
      return;
    }
