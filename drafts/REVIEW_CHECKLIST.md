...
Drafts are generated by the free template-based writer (`scripts/draft-from-anomaly.mjs`), which produces mechanical first drafts with zero API cost. The LLM drafter (`scripts/draft-from-anomaly-llm.mjs`) is available as an opt-in alternative when richer prose is worth the Anthropic spend.
