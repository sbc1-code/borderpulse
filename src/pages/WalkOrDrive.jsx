...
export default function WalkOrDrive() {
  ...
  useEffect(() => {
    ...
    const title = language === 'en'
      ? `Walk or drive across ${crossing.name}? CBP wait-report comparison | Border Pulse`
      : `¿Cruzar a pie o en auto en ${crossing.name}? Comparación de reportes de CBP | Border Pulse`;
    const desc = language === 'en'
      ? `Compare official CBP pedestrian and vehicle wait reports at ${crossing.name}. See whether walking may save enough time to be worth parking.`
      : `Compara reportes oficiales de espera de CBP a pie y en vehículo en ${crossing.name}. Mira si caminar podría ahorrar suficiente tiempo para que valga estacionar.`;
    ...
    return (
      <div className="p-3 sm:p-4 lg:p-6 max-w-[1100px] mx-auto">
        ...
        <p className="text-sm text-slate-700 dark:text-slate-300 mb-4">
          {language === 'en'
            ? `CBP doesn't publish a pedestrian wait time for ${crossing.name}, so the walk-vs-drive question can't be answered from data. Some ports don't have pedestrian lanes; others have them but don't report. Check the crossing page for the latest published report.`
            : `CBP no publica tiempo de espera peatonal para ${crossing.name}, así que la pregunta caminar-vs-manejar no se puede responder con datos. Algunos puertos no tienen carriles peatonales; otros sí pero no reportan. Revisa la página del cruce para el último reporte publicado.`}
        </p>
        <Link to={`/crossing/${canonicalSlug}/`} className="inline-flex items-center gap-1 text-sm font-medium text-emerald-700 dark:text-emerald-400 hover:underline">
          {language === 'en' ? `Open ${crossing.name} wait reports` : `Abrir reportes de espera de ${crossing.name}`} <ArrowRight className="w-4 h-4" />
        </Link>
      </div>
    );
  }
    ...
    <div className="p-3 sm:p-4 lg:p-6 max-w-[1100px] mx-auto">
      ...
      <header className="mb-4">
        ...
        <p className="text-xs sm:text-sm text-slate-500 mt-1 inline-flex items-center gap-1.5">
          ...
          {language === 'en'
            ? `${hasFreshData ? 'Current CBP wait reports' : `Latest CBP report (${dataStatusLabel})`}, plus ${FOOT_OVERHEAD_MIN} min added to the foot total to cover parking and walking the bridge. Northbound only.`
            : `${hasFreshData ? 'Reportes actuales de CBP' : `Último reporte de CBP (${dataStatusLabel})`}, más ${FOOT_OVERHEAD_MIN} min al total a pie para cubrir estacionar y cruzar el puente. Solo hacia EE.UU.`}
        </p>
      </header>
        ...
        <div className={`mb-4 rounded-lg border px-4 py-3 ${
          ...
          <div className={`text-[10px] uppercase tracking-wider font-semibold mb-0.5 ${
            ...
            {hasFreshData
              ...
              : (language === 'en' ? `Latest CBP report · ${dataStatusLabel}` : `Último reporte de CBP · ${dataStatusLabel}`)}
          </div>
          <p className="text-sm text-slate-900 dark:text-white">
            {(() => {
              ...
              if (r.verdict === 'walk') {
                ...
                return language === 'en'
                  ? `This CBP report favors walking: ${laneEn} plus ${FOOT_OVERHEAD_MIN} min for parking and the bridge totals ${r.footTotal} min, versus ${veh} min driving. Recheck before deciding.`
                  : `Este reporte de CBP favorece caminar: ${laneEs} más ${FOOT_OVERHEAD_MIN} min de estacionar y cruzar el puente suma ${r.footTotal} min, contra ${veh} min manejando. Verifica antes de decidir.`;
              }
              if (r.verdict === 'drive') {
                if (hasFreshData) {
                  return language === 'en'
                ...
                return language === 'en'
                  ? `This CBP report favors driving: vehicle wait is ${veh} min versus ${r.footTotal} min on foot (including ${FOOT_OVERHEAD_MIN} min parking + bridge). Recheck before deciding.`
                  : `Este reporte de CBP favorece manejar: la espera en auto es de ${veh} min contra ${r.footTotal} min a pie (incluyendo ${FOOT_OVERHEAD_MIN} min de estacionar + puente). Verifica antes de decidir.`;
              }
              if (hasFreshData) {
                return language === 'en'
              ...
              return language === 'en'
                ? `This CBP report is a near tie: vehicle wait is ${veh} min and foot total is ${r.footTotal} min (within ±${RECOMMEND_GAP_MIN}). Recheck before deciding.`
                : `Este reporte de CBP está parejo: espera en auto ${veh} min y total a pie ${r.footTotal} min (dentro de ±${RECOMMEND_GAP_MIN}). Verifica antes de decidir.`;
            })()}
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4">
        <PanelCard
          ...
          lang={language}
          recommendedLabel={hasFreshData && recommendation?.verdict === 'drive' ? (language === 'en' ? 'Recommended' : 'Recomendado') : null}
        />
        <PanelCard
          icon={User}
          ...
          lang={language}
          recommendedLabel={hasFreshData && recommendation?.verdict === 'walk' && recommendation?.via === 'standard' ? (language === 'en' ? 'Recommended' : 'Recomendado') : null}
        />
        <PanelCard
          icon={User}
          ...
          lang={language}
          recommendedLabel={hasFreshData && recommendation?.verdict === 'walk' && recommendation?.via === 'ready' ? (language === 'en' ? 'Recommended' : 'Recomendado') : null}
        />
      </div>
      ...
      <div className="mb-6 flex flex-wrap gap-3 text-sm">
        ...
        >
          {language === 'en' ? `${crossing.name} wait reports` : `Reportes de espera de ${crossing.name}`} <ArrowRight className="w-4 h-4" />
        </Link>
        <Link
