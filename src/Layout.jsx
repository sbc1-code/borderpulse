...
import { BarChart3, BookOpen, Menu, X, Moon, Sun, Bell, Code, ExternalLink, TrendingUp } from 'lucide-react';
...
export default function Layout({ children }) {
  ...
  const nav = [
    ...
    {
      name: language === 'en' ? 'Predictions' : 'Predicciones',
      href: '/predictions',
      icon: TrendingUp,
      current: location.pathname.startsWith('/predictions'),
    },
    {
      ...
      <div className="lg:flex min-h-screen">
        <aside className={`hidden lg:flex w-60 shrink-0 ${sidebarClasses} backdrop-blur-md border-r flex-col`}>
          <div className={`p-5 border-b ${isDark ? 'border-gray-800' : 'border-slate-200'}`}>
            ...
            {/* Language toggle in the sidebar header so it's always visible
                regardless of viewport / sidebar scroll. Mirrors the mobile
                header. The bottom-of-sidebar version was below the fold
                on tall pages. */}
